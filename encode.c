...
{
    fseek(fptr_src_image , 0 , SEEK_SET);         // rewind(fptr);
    char str[54];
...
Status encode_secret_file_extn(char *file_size, EncodeInfo *encInfo)
{
    encode_data_to_image(file_size, strlen(file_size),encInfo);
    return e_success;
...
Status do_encoding(EncodeInfo *encInfo)
{
    ...
    {
        printf("Failed to opne the faile\n");
        return e_failure;
    }
    ...
    {
        printf("Encoded secrete file extension size successfully\n");
    }  
    else
    {
        printf("Failed to encode secrete file extension size\n");
        return e_failure;
    }
    ...
    {
        printf("Failed to encode secrete file data\n");
        return e_failure;
    }

    if(copy_remaining_img_data(encInfo->fptr_src_image ,encInfo->fptr_stego_image) == e_success)
    {
        printf("Copying remaining image is successfull\n");
    }
    else
    {
